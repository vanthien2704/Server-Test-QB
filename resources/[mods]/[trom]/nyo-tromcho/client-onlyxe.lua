...
Citizen.CreateThread(function()
	while Authorized == false do
		QBCore.Functions.TriggerCallback('nyo-tromcho:Authorizing', function(callback)
			Authorized = callback
		end)
	...
	while true do
		...
		if Authorized then
			if Config.Framework == "QBCore" then 
				if QBCore.Functions.GetPlayerData().metadata["ishandcuffed"] or QBCore.Functions.GetPlayerData().metadata["isdead"] or QBCore.Functions.GetPlayerData().metadata["inlaststand"] then 
					if Holding then 
						if DoesEntityExist(CurrentHolding) then
			...
			if IsPlayerInDogZone() and Hours >= Config.MinTime and Hours <= Config.MaxTime then
				...
				if IsPedInAnyVehicle(PlayerPedId()) then
					if Config.PhuongTien.Allow then 
									...
									if dist <= Config.PhuongTien.Distance then
										...
										if IsControlJustPressed(1, 38) and not Catching and not Holding then
											...
											if CurrentCops >= Config.CopRequired then 
												QBCore.Functions.TriggerCallback(''..Config.Framework..':HasItem', function(result)
													if result == true then
														Catching = true
														...
														Skillbar.Start({
															...
															if SucceededAttempts + 1 >= Config.Skillbar.Solan then 
																...
																QBCore.Functions.Notify('Đã bắt thành công một con chó!', 'success')
															else 
															...
															if Config.ChoCan then 
																if math.random(1,10) <= Config.ChanceChoCan then 
																	...
																	QBCore.Functions.Notify('Bạn đã bị chó cắn!', 'error')
																	...
																	DeleteEntity(ped)
																	TriggerEvent('animations:client:EmoteCommandStart', {"c"})
																	QBCore.Functions.Notify('Chó đã chạy đi!', 'error')
																end
															else 
																local model = GetEntityModel(ped)
																SetEntityAsNoLongerNeeded(ped)
														...
														QBCore.Functions.Notify('Bạn không có thòng lọng!', 'error')
													end
												end, Config.PhuongTien.VatPham)
											else 
												QBCore.Functions.Notify("Không đủ cảnh sát ("..Config.CopRequired.." trở lên)", "error")
											end
										end 
									end 
								end
						...
						QBCore.Functions.Notify("Vui lòng xuống xe để bắt chó", "error")
					end							
				end
			end
		end
			...
			if #(Pos - vector3(Config.BanCho.ViTri.x, Config.BanCho.ViTri.y, Config.BanCho.ViTri.z)) <= 2 then		
				...
				if IsControlJustPressed(1, 38) then
					if Holding then 
						...
						QBCore.Functions.Notify("Bạn không ôm bất kỳ con chó nào", "error")
					end 
				end 
			end 
		end				
