...
function spawnCrate(coords)
  ...
  exports['qb-target']:AddTargetEntity(drop, {
    options = {
        {
            type = "client",
            event = "mad-airdrops:client:lootAirdrop",
            icon = "fa-solid fa-user-secret",
            label = "Loot Supplies"
        }
    },
...
RegisterNetEvent('mad-airdrops:client:lootAirdrop', function()
    local state = lib.callback.await('mad-airdrops:server:getLootState', false)
    if not state then
        TriggerServerEvent("mad-airdrops:server:sync:loot")
        if lib.progressBar({
...
RegisterNetEvent('mad-airdrops:client:clearStuff',function()
  ...
  exports['qb-target']:RemoveTargetEntity(drop)
end)
